...
import tae1_2 from './image/tae1_2.jpg';
...
const URL = "https://teachablemachine.withgoogle.com/models/Dm9W4OC3E/";
...
async function predict() {
    ...
    for (let i = 0; i < maxPredictions; i++) {
        if(prediction[i].className ==="Stand" && prediction[i].probability > 0.7){
            if((stand === "Straight_Right") || (stand === "Straight_Left")){
              stand = "Stand";
              count++;
            }
        }
        if(prediction[i].className ==="Straight_Right" && prediction[i].probability > 0.7){
            stand = "Straight_Right";
        }
        if(prediction[i].className ==="Straight_Left" && prediction[i].probability > 0.7){
            stand = "Straight_Left";
      }
    }
      ...
      return (
        <Container>
            ...
            <div>
              ...
              <div>
              <img src={tae1_2} alt="Tae1_2"/>
              <h2>태권도 - 주먹 지르기</h2>
              <h3>1. 양 발을 자신의 어깨 넓이보다 조금 더 넓게 벌린 후 무릎을 살짝 굽히며, 양 손을 주먹 쥔 채 자신의 허리 옆으로 가져간다.</h3>
              <h3>2. 오른팔부터 앞을 향해 자신의 눈 높이까지 빠르게 뻗는다.</h3>
              <h3>3. 뻗은 오른팔을 거두며 왼팔을 뻗는다.</h3>
