...
import tae2 from './image/tae2.jpg';
...
async function predict() {
    ...
    for (let i = 0; i < maxPredictions; i++) {
        if(prediction[i].className ==="Stand" && prediction[i].probability > 0.7){
            if((stand === "OneHandSide_R") || (stand === "OneHandSide_L")){
              stand = "Stand";
              count++;
            }
        }
        if(prediction[i].className ==="OneHandSide_R" && prediction[i].probability > 0.7){
            stand = "OneHandSide_R";
        }
        if(prediction[i].className ==="OneHandSide_L" && prediction[i].probability > 0.7){
            stand = "OneHandSide_L";
      }
    }
...
class Tae2 extends React.Component{
  ...
  render() {
      ...
      return (
        <Container>
              ...
              <div>
              <img src={tae2} alt="tae2"/>
              </div>
              <h2>태권도 - 한 손날 몸통치기</h2>
              <h3>1. 양 발을 자신의 어깨 넓이보다 조금 더 넓게 벌린 후 무릎을 살짝 굽히며, 양 손을 주먹 쥔 채 자신의 허리 옆으로 가져간다.</h3>
              <h3>2. 오른팔부터 측면을 향해 자신의 눈 높이까지 빠르게 뻗는다. 이 때 손바닥은 아래로 향하게 하며 손을 펼친다.</h3>
              <h3>3. 뻗은 팔을 거두며 왼팔을 뻗는다.</h3>
